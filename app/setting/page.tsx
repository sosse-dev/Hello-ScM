...
import { handleDeleteUser } from "@/lib/handleDeleteUser";
...
function ProfilPage() {
  ...
  return (
    <>
      ...
      {deleteMenu && (
        <div className="absolute right-0 left-0 bottom-10 top-0 m-auto w-[23rem] h-fit border-[1.4px] bg-slate-50 border-black shadow-xl rounded-lg overflow-hidden flex flex-col">
          <h1 className="bg-slate-100 p-3 text-xl">Warning!</h1>
          <p className="bg-slate- px-3 py-2 md:text-sm">
            Do you want to delete your account?. Please click {"'"}Delete My
            Account{"'"} to continue
          </p>
          <div className="w-full h-fit py-2 flex justify-end gap-x-2 pr-4">
            <button
              onClick={() => {
                handleDeleteUser(session?.user.email as string),
